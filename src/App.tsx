...
const FB_PROJECT = "jv-ortopedia";
const FB_API_KEY = "AIzaSyDix_z2aZrR1270ElWlMkrWxkj_FKBDQTY";
const FB_URL = `https://firestore.googleapis.com/v1/projects/${FB_PROJECT}/databases/(default)/documents`;

// SDK de Firebase, usado solo para Storage (el resto de la app sigue usando fetch directo a Firestore)
const firebaseApp = initializeApp({
    apiKey: FB_API_KEY,
    authDomain: `${FB_PROJECT}.firebaseapp.com`,
    projectId: FB_PROJECT,
    storageBucket: `${FB_PROJECT}.firebasestorage.app`,
});
const storage = getStorage(firebaseApp);

// Sube un archivo a Firebase Storage y devuelve la URL pública de descarga
const uploadFile = async (file, carpeta) => {
    if (!file) return "";
    const nombreUnico = `${carpeta}/${Date.now()}_${file.name}`;
...
const fbGet = async () => {
    const r = await fetch(`${FB_URL}/pedidos?orderBy=createdAt&pageSize=100&key=${FB_API_KEY}`);
    ...
    return data.documents.map(d => {
        ...
        return {
            id, patient: str("patient"), telefono: str("telefono"), localidad: str("localidad"),
            age: num("age"), calzado: str("calzado"),
            largoDer: str("largoDer"), anchoDer: str("anchoDer"),
            largoIzq: str("largoIzq"), anchoIzq: str("anchoIzq"),
            photos: num("photos"), status: str("status"), date: str("date"),
            notes: str("notes"), weight: num("weight"), height: num("height"),
            pain: bool("pain"), painZone: str("painZone"),
        };
    });
};

const fbAdd = async (data) => {
    const fields = {};
    ...
    await fetch(`${FB_URL}/pedidos?key=${FB_API_KEY}`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fields }),
    });
};

const fbUpdateStatus = async (id, status) => {
    await fetch(`${FB_URL}/pedidos/${id}?updateMask.fieldPaths=status&key=${FB_API_KEY}`, {
...
const initialOrders = [
    { id: 1025, patient: "Juan Pérez", date: "20/05/2024", status: "Nuevo", age: 32, weight: 78, height: 175, pain: true, painZone: "Talón", calzado: "42", largoDer: "26.5", anchoDer: "9.5", largoIzq: "26.0", anchoIzq: "9.0", photos: 2, notes: "" },
    { id: 1024, patient: "María Gómez", date: "20/05/2024", status: "En revisión", age: 45, weight: 65, height: 162, pain: true, painZone: "Arco plantar", calzado: "37", largoDer: "23.5", anchoDer: "8.5", largoIzq: "23.0", anchoIzq: "8.0", photos: 2, notes: "" },
    { id: 1023, patient: "Carlos Ruiz", date: "19/05/2024", status: "En fabricación", age: 28, weight: 82, height: 180, pain: false, painZone: "", calzado: "44", largoDer: "28.0", anchoDer: "10.0", largoIzq: "28.0", anchoIzq: "10.0", photos: 2, notes: "" },
    { id: 1022, patient: "Ana López", date: "19/05/2024", status: "Listo", age: 55, weight: 70, height: 158, pain: true, painZone: "Metatarso", calzado: "38", largoDer: "24.0", anchoDer: "9.0", largoIzq: "24.0", anchoIzq: "9.0", photos: 2, notes: "Urgente" },
    { id: 1021, patient: "Pedro Díaz", date: "18/05/2024", status: "Entregado", age: 38, weight: 90, height: 177, pain: true, painZone: "Talón", calzado: "43", largoDer: "27.5", anchoDer: "10.0", largoIzq: "27.5", anchoIzq: "9.8", photos: 2, notes: "" },
];
...
// Logo: dark=true para fondo blanco, dark=false para fondo oscuro
                ...
                {data.pedidoMedico === true && (
                    <label htmlFor="receta-upload" style={{ cursor: "pointer", display: "block" }}>
                        <div style={{ border: `2px dashed \${data.recetaFile ? C.green : C.teal}`, borderRadius: 12, padding: "16px", textAlign: "center", background: data.recetaFile ? `\${C.green}08` : `\${C.teal}05` }}>
                            ...
                            <div style={{ fontWeight: 700, color: data.recetaFile ? C.green : C.teal, fontSize: 14 }}>{data.recetaFile === "Subiendo..." ? "Subiendo..." : data.recetaFile ? "✓ Receta cargada" : "Subir receta médica"}</div>
                            <div style={{ fontSize: 11, color: C.gray, marginTop: 4 }}>JPG, PNG o PDF</div>
                        </div>
                        <input type="file" id="receta-upload" accept="image/*,.pdf" style={{ display: "none" }} onChange={async e => {
                            const file = e.target.files[0];
                            if (!file) return;
                            setData({ ...data, recetaFile: "Subiendo..." });
                            const url = await uploadFile(file, "recetas");
                            setData({ ...data, recetaFile: url });
                        }} />
                    </label>
                )}
            </div>
...
// PASO 2 — Medidas: largo + ancho metatarso para cada pie
        ...
        <div style={{ background: C.bg, borderRadius: 14, padding: 16, marginBottom: 14, border: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
                <svg width={110} height={145} viewBox="0 0 110 145">
                    ...
                    {/* flecha ANCHO rojo al metatarso */}
    ...
    return (
        <div>
            ...
            <div style={{ background: "#FFF8E0", border: "1px solid #FFD97D", borderRadius: 12, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#7D5A00" }}>
                ⚠️ <strong>Importante:</strong> medí ambos pies. Se usa el más largo y más ancho como referencia para fabricar la plantilla.
            </div>
...
// PASO 3 — 2 fotos: ambos pies arriba + ambos pies atrás
function Step3({ data, setData, onNext, onBack }) {
    const photos = [
        { key: "fotoArriba", label: "Ambos pies juntos", sublabel: "Vista de arriba", icon: "👆", desc: "Poné ambos pies sobre fondo blanco y fotografiá desde arriba, perpendicular al piso" },
        { key: "fotoAtras", label: "Ambos pies juntos", sublabel: "Vista de atrás", icon: "🔙", desc: "Parate y fotografiá desde atrás mostrando ambos talones al mismo tiempo" },
    ];
        ...
        <div>
            ...
            {photos.map(p => (
                <label key={p.key} htmlFor={p.key} style={{ cursor: "pointer", display: "block", marginBottom: 14 }}>
                    <div style={{ borderRadius: 16, border: `2px dashed ${data[p.key] ? C.green : C.border}`, background: data[p.key] ? `${C.green}08` : C.white, padding: "18px 16px", display: "flex", alignItems: "center", gap: 16, transition: "all .2s", boxShadow: "0 2px 10px rgba(0,0,0,.05)" }}>
...
function Step4({ formData, onSubmit, onBack, saving }) {
    ...
    const secs = [
        ...
        { title: "📋 Pedido médico", rows: [["¿Tiene pedido médico?", s1.pedidoMedico === true ? "✓ Sí" : s1.pedidoMedico === false ? "No" : "-"], ["Receta", s1.pedidoMedico ? (s1.recetaFile ? "✓ Receta cargada" : "Sin archivo") : "No aplica"]] },
...
function UserFlow({ onBack }) {
    ...
    const handleSubmit = () => {
        ...
        const msg = [
            ...
            "Vista posterior: " + (s3.fotoAtras ? "Adjunta" : "No adjunta"),
        ].join("\n");
...
function generatePDF(order) {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pedido #${order.id} - J&V Ortopedia</title>
  <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;color:#0D2B55;padding:30px}
  .hdr{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #1A7DAF;padding-bottom:18px;margin-bottom:22px}
  .logo{font-size:26px;font-weight:900;letter-spacing:-1px;color:#0D2B55}.sub{font-size:11px;color:#1A7DAF;font-weight:700;letter-spacing:2px;margin-top:2px}
  .num{font-size:20px;font-weight:900;text-align:right}.badge{display:inline-block;background:#E8F4FD;color:#1A7DAF;border:1px solid #A8D5F0;border-radius:20px;padding:4px 14px;font-size:12px;font-weight:700;margin-top:4px}
  .sec{margin-bottom:20px}.st{background:#0D2B55;color:#fff;padding:9px 16px;border-radius:8px 8px 0 0;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.5px}
  ...
  .fg{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:14px 16px;border:1px solid #D0DDE9;border-top:none;border-radius:0 0 8px 8px}
  .fb{border:1.5px dashed #D0DDE9;border-radius:10px;padding:24px 10px;text-align:center;font-size:12px;color:#6B7E99;line-height:1.7}
  ...
  <div class="hdr">
    ...
    <div><div class="num">Pedido #${order.id}</div><div class="badge">${order.status}</div><div style="font-size:11px;color:#6B7E99;margin-top:5px">Fecha: ${order.date}</div></div>
  </div>
  <div class="sec"><div class="st">Datos del paciente</div><div class="sb">
    ...
    <div class="row"><span class="lbl">Edad</span><span class="val">${order.age} años</span></div>
    <div class="row"><span class="lbl">Peso / Altura</span><span class="val">${order.weight} kg — ${order.height} cm</span></div>
    <div class="row"><span class="lbl">Dolor / zona</span><span class="val">${order.pain ? "Si — " + order.painZone : "No refiere dolor"}</span></div>
  </div></div>
  <div class="sec"><div class="st">Calzado y medidas</div><div class="sb">
    <div class="cz"><div><div style="font-size:11px;color:#6B7E99;font-weight:600">NUMERO DE CALZADO</div><div style="font-size:22px;font-weight:900;color:#0D2B55">N° ${order.calzado}</div></div></div>
    <div class="grid2">
    ...
    <div class="ref">Medida de referencia para fabricar: Largo ${refL} cm x Ancho metatarso ${refA} cm</div>
  </div></div>
  ...
  <div class="ft"><span>J&V Ortopedia — Plantillas ortopedicas a medida</span><span>Pedido #${order.id} — ${order.date}</span></div>
  </body></html>`;
    const blob = new Blob([html], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `Pedido-${order.id}-${order.patient.replace(/ /g, "_")}.html`;
...
function AdminPanel({ onBack }) {
    ...
    const loadOrders = async () => {
        try { const data = await fbGet(); setOrders(data); } catch (e) { }
        setLoading(false);
    };
    ...
    const filtered = orders.filter(o => (filterStatus === "Todos" || o.status === filterStatus) && (o.patient.toLowerCase().includes(search.toLowerCase()) || String(o.id).includes(search)));
    const updateStatus = async (id, s) => {
        await fbUpdateStatus(id, s);
    ...
    return (
        <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
            <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: "18px" }}>
            ...
            {loading && <div style={{ textAlign: "center", padding: "30px 0", color: C.gray, fontSize: 14 }}>Cargando pedidos...</div>}
            ...
            <div style={{ padding: "12px 14px 0" }}>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  Buscar paciente o N° pedido..." style={{ width: "100%", padding: "11px 16px", borderRadius: 12, border: `1.5px solid ${C.border}`, fontSize: 14, color: C.navy, boxSizing: "border-box", outline: "none", fontFamily: "inherit" }} />
            </div>
            <div style={{ display: "flex", gap: 8, padding: "10px 14px", overflowX: "auto" }}>
                {ALL_STATUSES.map(s => <button key={s} onClick={() => setFilterStatus(s)} style={{ padding: "6px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 800, whiteSpace: "nowrap", background: filterStatus === s ? C.navy : C.white, color: filterStatus === s ? "#fff" : C.gray, boxShadow: "0 2px 6px rgba(0,0,0,.06)", transition: "all .2s" }}>{s}</button>)}
            </div>
            <div style={{ padding: "0 14px 100px" }}>
                {filtered.length === 0 && <div style={{ textAlign: "center", padding: "40px 0", color: C.gray }}><div style={{ fontSize: 40 }}>📭</div><div style={{ fontWeight: 700, marginTop: 10 }}>Sin resultados</div></div>}
                {filtered.map(order => (
                    <div key={order.id} onClick={() => { setSelected(order); setSideOpen(true); }} style={{ background: C.white, borderRadius: 16, padding: "14px", marginBottom: 10, boxShadow: "0 2px 10px rgba(0,0,0,.06)", cursor: "pointer", border: `1px solid ${C.border}` }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                            <div><div style={{ fontWeight: 800, color: C.navy, fontSize: 15 }}>{order.patient}</div><div style={{ fontSize: 12, color: C.gray }}>#{order.id} · {order.date} · Calzado N° {order.calzado}</div></div>
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            ...
                            <span style={{ fontSize: 11, background: C.grayLight, color: C.gray, borderRadius: 6, padding: "3px 8px", fontWeight: 600 }}>📸 {order.photos} fotos</span>
                            {order.notes && <span style={{ fontSize: 11, background: "#FFF3E0", color: C.orange, borderRadius: 6, padding: "3px 8px", fontWeight: 600 }}>⚠ {order.notes}</span>}
                        </div>
                    </div>
                ))}
            </div>
                ...
                <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", zIndex: 100 }} onClick={() => setSideOpen(false)}>
                    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: C.white, borderRadius: "22px 22px 0 0", maxHeight: "90vh", overflowY: "auto", padding: "20px 18px 32px" }} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                            <div><div style={{ fontWeight: 900, fontSize: 18, color: C.navy }}>{selected.patient}</div><div style={{ fontSize: 12, color: C.gray }}>Pedido #{selected.id} · {selected.date}</div></div>
                            <Badge status={selected.status} />
                        </div>
                        <button onClick={() => generatePDF(selected)} style={{ width: "100%", padding: "13px", borderRadius: 12, border: "none", background: "linear-gradient(135deg,#E74C3C,#C0392B)", color: "#fff", fontWeight: 800, fontSize: 14, cursor: "pointer", marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>📄 Generar PDF del pedido</button>
                        <div style={{ background: C.bg, borderRadius: 14, padding: 14, marginBottom: 14 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 16px" }}>
                                {[["Edad", `${selected.age} años`], ["Peso", `${selected.weight} kg`], ["Altura", `${selected.height} cm`], ["Dolor", selected.pain ? selected.painZone : "No"]].map(([k, v]) => (
                        ...
                        <div style={{ background: `${C.navy}08`, border: `1.5px solid ${C.navy}20`, borderRadius: 14, padding: 14, marginBottom: 14, display: "flex", alignItems: "center", gap: 14 }}>
                            <span style={{ fontSize: 28 }}>👟</span>
                            <div><div style={{ fontSize: 11, color: C.gray, fontWeight: 600 }}>CALZADO</div><div style={{ fontSize: 24, fontWeight: 900, color: C.navy }}>N° {selected.calzado}</div></div>
                        </div>
                        <div style={{ background: C.bg, borderRadius: 14, overflow: "hidden", marginBottom: 14 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                                {[["derecho", ["largoDer", "anchoDer"]], ["izquierdo", ["largoIzq", "anchoIzq"]]].map(([lado, keys]) => (
                                    <div key={lado} style={{ padding: "12px 14px", borderRight: lado === "derecho" ? `1px solid ${C.border}` : "none" }}>
                                        ...
                                        {[["Largo", 0], ["Ancho metatarso", 1]].map(([label, idx]) => (
                            ...
                            <div style={{ padding: "10px 14px", background: `${C.green}10`, borderTop: `1px solid ${C.green}30` }}>
                                <span style={{ fontSize: 12, color: C.greenDark, fontWeight: 700 }}>✅ Referencia: {Math.max(parseFloat(selected.largoDer) || 0, parseFloat(selected.largoIzq) || 0).toFixed(1)} cm × {Math.max(parseFloat(selected.anchoDer) || 0, parseFloat(selected.anchoIzq) || 0).toFixed(1)} cm</span>
                            </div>
                        </div>
                        <div style={{ marginBottom: 14 }}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: C.navy, marginBottom: 10 }}>📸 Fotos ({selected.photos})</div>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                                {["Ambos pies — Arriba", "Ambos pies — Atrás"].map(l => (
                                    <div key={l} style={{ background: C.grayLight, borderRadius: 12, aspectRatio: "4/3", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6 }}>
                                        <span style={{ fontSize: 26 }}>🦶</span><span style={{ fontSize: 10, color: C.gray, textAlign: "center", padding: "0 6px" }}>{l}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        {selected.notes && <div style={{ background: "#FFF8E0", border: "1px solid #FFD97D", borderRadius: 12, padding: "12px 14px", marginBottom: 16 }}><span style={{ fontWeight: 700, color: C.orange }}>⚠ </span><span style={{ fontSize: 13, color: C.navy }}>{selected.notes}</span></div>}
...
function AdminLogin({ onSuccess, onBack }) {
    ...
    return (
        <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
            <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: "28px 20px 80px" }}>
                ...
                <p style={{ color: "rgba(255,255,255,.7)", fontSize: 13, margin: 0 }}>Ingresá tu contraseña para continuar</p>
            </div>
            <div style={{ margin: "-52px 20px 0", background: C.white, borderRadius: 22, padding: "32px 22px", boxShadow: "0 8px 40px rgba(0,0,0,.10)" }}>
                ...
                <div style={{ marginBottom: 20 }}>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: C.navy, marginBottom: 6, textTransform: "uppercase", letterSpacing: .5 }}>Contraseña</label>
                    ...
                    {error && <div style={{ color: C.red, fontSize: 13, fontWeight: 600, marginTop: 8 }}>❌ Contraseña incorrecta, intentá de nuevo.</div>}
                </div>
...
// HOME — sin botón admin visible; acceso con 7 toques en el logo
function Home({ onUser, onAdmin }) {
    ...
    return (
        <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
            <div style={{ background: `linear-gradient(150deg, ${C.navy} 0%, ${C.navyLight} 55%, ${C.teal} 100%)`, padding: "52px 28px 68px", textAlign: "center" }}>
                ...
                <div style={{ marginBottom: 28 }}>
                    ...
                    {["Completá tus datos personales", "Ingresá las medidas de tus pies", "Sacá 2 fotos guiadas", "Revisá y enviá tu pedido"].map((s, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                            <div style={{ width: 30, height: 30, borderRadius: "50%", background: `linear-gradient(135deg, ${C.teal}, ${C.navyLight})`, color: "#fff", fontSize: 13, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{i + 1}</div>
